...
int main(int argc, char **argv){
    ...
    Vec6 _bias;
    #if GAIT_DEFAULT_TROT
        _bias << 0, 0.5, 0.5, 0, 0, 0.5;// Trot
    #endif
    # if GAIT_DEFAULT_WALK
        // walk 20250401
        _bias << 0.9999, 0.3333, 
                0.6666, 0.6666, 
                0.3333, 0.9999;
    #endif
    # if GAIT_DEFAULT_CLAW
        // claw 20250405
        _bias << 0.6666, 0.1666, 
                 0.8333, 0.3333, 
                 0.9999, 0.4999;
    ...
    #if USE_A_REAL_HEXAPOD == false
        #if GAIT_DEFAULT_TROT
            ctrlComp->waveGen = new WaveGenerator(0.55, 0.52, _bias); // Trot
        #endif
        #if GAIT_DEFAULT_WALK
            ctrlComp->waveGen = new WaveGenerator(1, 0.6667, _bias); // walk 20250401
        #endif
        #if GAIT_DEFAULT_CLAW
            ctrlComp->waveGen = new WaveGenerator(1.8, 0.8334, _bias); // claw 20250405
        #endif
    #else
        #if GAIT_DEFAULT_TROT
        ctrlComp->waveGen = new WaveGenerator(0.75, 0.52, _bias); // Trot
        #endif
        #if GAIT_DEFAULT_WALK
            ctrlComp->waveGen = new WaveGenerator(1.125, 0.6667, _bias); // walk 20250401
        #endif
        #if GAIT_DEFAULT_CLAW
            ctrlComp->waveGen = new WaveGenerator(2.25, 0.8334, _bias); // claw 20250405
        #endif
    #endif
