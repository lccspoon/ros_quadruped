...
bool State_VMC::checkStepOrNot(){
    ...
    //         (fabs(_posError(0)) > 0.02) ||
    //         (fabs(_posError(1)) > 0.02) ||
    //         (fabs(_velError(0)) > 0.01) ||
    //         (fabs(_velError(1)) > 0.01) ||
    //         (fabs(_dYawCmd) > 0.10) ){
    //         return true;
    //     }
    //     else{
...
void State_VMC::calcQQd(){
    ...
    for(int i(0); i<4; ++i){
        ...
        // _posFeet2BGoal.col(i) = _G2B_RotMat * (_posFeetGlobalGoal.col(i) - Vec3(0, 0, 0)); // lcc 20240621

        _velFeet2BGoal.col(i) = _G2B_RotMat * (_velFeetGlobalGoal.col(i) - _velBody); 
        // _velFeet2BGoal.col(i) = _G2B_RotMat * (_velFeetGlobalGoal.col(i) - Vec3(0, 0, 0)); // lcc 20240621
