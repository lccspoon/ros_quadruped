...
FSM::FSM(CtrlComponents *ctrlComp)
    ...
    _stateList.force_pos_WBC = new State_Force_Pos_WBC(_ctrlComp);//lcc 20250405
...
void FSM::run(){
    // usleep(10000); lcc 20250405 注意这里的延时在仿真中会崩溃，在实物中可能也会影响效果，需要进一步实验验证
        ...
        if( MOTOR_DISABEL_FLAG == true ){ // 'o'
            ...
            for (int i = 0; i < 18; i++)
            {
                ...
                _ctrlComp->lowCmd->setAllLegGain(50, 1);
            }
...
void FSM::fsm_run(){
    ...
    // std::cout<<" cpg_scheduler.blcok(1,6,0,0): \n"<< cpg_scheduler.block(0, 0, 1, 6) <<std::endl;
    ...
    cpg_scheduler.col(0) = cpg_scheduler_temp.col(0);
    cpg_scheduler.col(1) = cpg_scheduler_temp.col(5);
    cpg_scheduler.col(2) = cpg_scheduler_temp.col(1);
    cpg_scheduler.col(3) = cpg_scheduler_temp.col(4);
    cpg_scheduler.col(4) = cpg_scheduler_temp.col(2);
    cpg_scheduler.col(5) = cpg_scheduler_temp.col(3);

    // std::cout<<" cpg_scheduler: \n"<< cpg_scheduler <<std::endl;
    // std::cout<<" cpg_scheduler.blcok(1,6,0,0): \n"<< cpg_scheduler.block(0, 0, 1, 6) <<std::endl;

    //lcc 20250405加入cpg
    _ctrlComp->runWaveGen(cpg_scheduler.block(0, 0, 1, 6).transpose(), cpg_scheduler.block(1, 0, 1, 6).transpose()); 

    // std::cout<<" _ctrlComp->contact_hex(0):"<< (*_ctrlComp->contact_hex)(0) <<std::endl;
...
FSMState* FSM::getNextState(FSMStateName stateName){
    ...
    case FSMStateName::FORCE_POS:    //lcc 20240827
        ...
        break;
    case FSMStateName::FORCE_POS_WBC:    //lcc 20250405
        return _stateList.force_pos_WBC;
        break;
