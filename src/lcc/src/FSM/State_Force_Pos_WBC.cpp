#include "FSM/State_Force_Pos_WBC.h"
#include "interface/KeyBoard.h"
#include <iomanip>

State_Force_Pos_WBC::State_Force_Pos_WBC(CtrlComponents *ctrlComp)
             :FSMState(ctrlComp, FSMStateName::FORCE_POS_WBC, "force_pos_wbc"), 
...
State_Force_Pos_WBC::~State_Force_Pos_WBC(){
    delete _gait;
    delete _gait_P;
}

void State_Force_Pos_WBC::enter(){
    ...
    // std::cout<<" _ctrlComp->waveGen->ret_period() :"<< _ctrlComp->waveGen->ret_period() <<std::endl;
    // std::cout<<" _ctrlComp->waveGen->ret_stancePhaseRatio() :"<< _ctrlComp->waveGen->ret_stancePhaseRatio() <<std::endl;
    // std::cout<<" _ctrlComp->waveGen->ret_bias() :"<< _ctrlComp->waveGen->ret_bias() <<std::endl;

    current_period = _ctrlComp->waveGen->ret_period();
    current_stancePhaseRatio = _ctrlComp->waveGen->ret_stancePhaseRatio();
    current_bias = _ctrlComp->waveGen->ret_bias();


    current_bias_part1 = current_bias.block<3, 1>(0, 0);
    current_bias_part2 = current_bias.block<3, 1>(3, 0);
}

void State_Force_Pos_WBC::exit(){
    _ctrlComp->ioInter->zeroCmdPanel();
    _ctrlComp->setAllSwing();
}

FSMStateName State_Force_Pos_WBC::checkChange(){
    if(_lowState->userCmd == UserCommand::PASSIVE_1){
        ...
        return FSMStateName::FORCE_POS_WBC;
    }
}

void State_Force_Pos_WBC::run(){
    // Rob State
    ...
    root_euler_d(0) = root_euler_d(0) / ( 1 + 2.5 * fabs( root_euler_d(0) ) ); //lcc ASSF
    root_euler_d(1) = root_euler_d(1) / ( 1 + 2.5 * fabs( root_euler_d(1) ) ); //lcc ASSF
    // root_euler_d(1) = root_euler_d(1) * 0.35;
    // A
    ...
    for(int i(0); i<6; ++i){
        if((*_contact_hex)(i) == 0){
            ...
            _lowCmd->setLegGain(i, 100 * kkk, 1);//stand 20250401
        }
    }
    ...
    gaitChange();// lcc 20250416
}

bool State_Force_Pos_WBC::checkStepOrNot(){
...
void State_Force_Pos_WBC::getUserCmd(){
    /* Movement */
...
void State_Force_Pos_WBC::calcCmd(){
    /* Movement */
...
void State_Force_Pos_WBC::calcTau(){
...
void State_Force_Pos_WBC::calc_wbc(void){
/*
    creatd by lcc 20250414
*/
    Eigen::Matrix<double, 25, 1> _q_with_quat; 
    Eigen::Matrix<double, 24, 1> _q_with_rpy;
    Eigen::Matrix<double, 24, 1> _qd;
    Eigen::Matrix<int,6, 1> _contact_hexa;


    _q.setZero();
    _qd.setZero();
    _contact_hexa.setZero();
    ...
    Eigen::Matrix<double, 3, 1> x_2 = rpy;
    Eigen::Matrix<double, 3, 1> x_2_d = root_euler_d;
    Eigen::Matrix<double, 3, 1> xd_2 = _wCmdGlobal;
    Eigen::Matrix<double, 3, 1> xd_2_d = _lowState->getGyro();

    Eigen::Matrix<double, 3, 24> J_2;
    J_2.setZero();
    J_2.block<3, 3>(0, 0) = _ctrlComp->lowState->getRotMat();

    //task3 机身平动：n=3
    Eigen::Matrix<double, 3, 1> x_3 = _posBody;
    Eigen::Matrix<double, 3, 1> x_3_d = _pcd;
    Eigen::Matrix<double, 3, 1> xd_3 = _velBody;
    Eigen::Matrix<double, 3, 1> xd_3_d = _vCmdGlobal;
    Eigen::Matrix<double, 3, 24> J_3;
    J_3.setZero();
    J_3.block<3, 3>(0, 3) = _ctrlComp->lowState->getRotMat();
    ...
    if( (*_contact_hex)(0) == 0 ){ // 0号腿摆动 0 3 4
        x_4.block<3, 1>(0, 0) = _posFeetGlobal.col(0);
        x_4_d.block<3, 1>(0, 0) = _posFeetGlobalGoal.col(0);
        xd_4.block<3, 1>(0, 0) = _velFeetGlobal.col(0);
        xd_4_d.block<3, 1>(0, 0) = _velFeetGlobalGoal.col(0);

        x_4.block<3, 1>(3, 0) = _posFeetGlobal.col(3);
        x_4_d.block<3, 1>(3, 0) = _posFeetGlobalGoal.col(3);
        xd_4.block<3, 1>(3, 0) = _velFeetGlobal.col(3);
        xd_4_d.block<3, 1>(3, 0) = _velFeetGlobalGoal.col(3);

        x_4.block<3, 1>(6, 0) = _posFeetGlobal.col(4);
        x_4_d.block<3, 1>(6, 0) = _posFeetGlobalGoal.col(4);
        xd_4.block<3, 1>(6, 0) = _velFeetGlobal.col(4);
        xd_4_d.block<3, 1>(6, 0) = _velFeetGlobalGoal.col(4);
        // _forceFeetGlobal.col(i) = _KpSwing*(_posFeetGlobalGoal.col(i) - _posFeetGlobal.col(i)) + _KdSwing*(_velFeetGlobalGoal.col(i)-_velFeetGlobal.col(i));
    }
    else if( (*_contact_hex)(1) == 0 ){ // 1号腿摆动 1 2 5
        x_4.block<3, 1>(0, 0) = _posFeetGlobal.col(1);
        x_4_d.block<3, 1>(0, 0) = _posFeetGlobalGoal.col(1);
        xd_4.block<3, 1>(0, 0) = _velFeetGlobal.col(1);
        xd_4_d.block<3, 1>(0, 0) = _velFeetGlobalGoal.col(1);

        x_4.block<3, 1>(3, 0) = _posFeetGlobal.col(2);
        x_4_d.block<3, 1>(3, 0) = _posFeetGlobalGoal.col(2);
        xd_4.block<3, 1>(3, 0) = _velFeetGlobal.col(2);
        xd_4_d.block<3, 1>(3, 0) = _velFeetGlobalGoal.col(2);

        x_4.block<3, 1>(6, 0) = _posFeetGlobal.col(5);
        x_4_d.block<3, 1>(6, 0) = _posFeetGlobalGoal.col(5);
        xd_4.block<3, 1>(6, 0) = _velFeetGlobal.col(5);
        xd_4_d.block<3, 1>(6, 0) = _velFeetGlobalGoal.col(5);
    }

    f_wbc = wbc.wbc_run( 
        _q_with_quat, _q_with_rpy, _qd, _contact_hexa,
        x_2, x_2_d, xd_2, xd_2_d, J_2,
        x_3, x_3_d, xd_3, xd_3_d, J_3,
        x_4, x_4_d, xd_4, xd_4_d,
        _forceFeetGlobal
    );
    ...
    else if ( wbc.ret_q_cmd().hasNaN() || wbc.ret_qd_cmd().hasNaN() ) 
    {
        tau_wbc = _sixlegdogModel->getTau(  _sixlegdogModel->getTau(_q, vec18ToVec36(f_wbc)), vec18ToVec36(f_wbc));
    }
    else
    {
        tau_wbc = _sixlegdogModel->getTau(  _sixlegdogModel->getTau(_q, vec18ToVec36(f_wbc)), vec18ToVec36(f_wbc)) + 
                    kp_wbc * ( wbc.ret_q_cmd()  - _q ) + 
                    kp_wbc * ( wbc.ret_qd_cmd() -  vec36ToVec18(_lowState->getQd_Hex()));  
    }
    // std::cout << "f_wbc: " << f_wbc.transpose() << std::endl;
    // std::cout << "wbc.ret_qd_cmd(): " << wbc.ret_qd_cmd().transpose() << std::endl;
    // std::cout << "wbc.ret_q_cmd(): " << wbc.ret_q_cmd().transpose() << std::endl;           
    // std::cout << "tau_wbc: " << vec18ToVec36(tau_wbc) << std::endl;           
}

void State_Force_Pos_WBC::calcP(){
...
Vec36 State_Force_Pos_WBC::_calcOP(float row, float pitch, float yaw, float height){
...
void State_Force_Pos_WBC::_torqueCtrl(){
    #if USE_A_REAL_HEXAPOD == true
...
void State_Force_Pos_WBC::gaitChange(void)
{
    #if USE_A_REAL_HEXAPOD == true
        // Trot_period = 0.8;
        // walk_period = 1.2;
...
void State_Force_Pos_WBC::gaitChange_MIT(void)
{
    duaration = 200;

    if( GAIT_INDEX == 0 )
    {
        _CPG.gaitChange( 0.5, 0.002, Trot_period );

        Eigen::Matrix<double, 6, 1> set_bias;
        set_bias.setZero();
        set_bias << 0, 0.5, 0.5, 0, 0, 0.5;// Trot
        set_bias_part1 << 0, 0.5, 0.5;
        set_bias_part2 << 0, 0, 0.5;

        current_period = linear_trans_period.linearConvert( current_period, Trot_period, duaration );
        current_stancePhaseRatio = linear_trans_stancePhaseRatio.linearConvert( current_stancePhaseRatio, 0.52, duaration );

        current_bias_part1 = linear_trans_bias_part1.linearConvert( current_bias_part1, set_bias_part1, duaration );
        current_bias_part2 = linear_trans_bias_part2.linearConvert( current_bias_part2, set_bias_part2, duaration );
        current_bias.block<3, 1>(0, 0) = current_bias_part1;
        current_bias.block<3, 1>(3, 0) = current_bias_part2;
    }
    else if( GAIT_INDEX == 1 )
    {

        _CPG.gaitChange( 0.666, 0.002, walk_period );

        Eigen::Matrix<double, 6, 1> set_bias;
        set_bias.setZero();
        set_bias << 0.9999, 0.3333, 
        0.6666, 0.6666, 
        0.3333, 0.9999;
        ...
        current_period = linear_trans_period.linearConvert( current_period, walk_period, duaration );
        current_stancePhaseRatio = linear_trans_stancePhaseRatio.linearConvert( current_stancePhaseRatio, 0.6667, duaration );
    ...
    else if( GAIT_INDEX == 2 )
    {
        Eigen::Matrix<double, 6, 1> set_bias;
        set_bias.setZero();
        set_bias << 0.6666, 0.1666, 
        0.8333, 0.3333, 
        0.9999, 0.4999;
        ...
        current_period = linear_trans_period.linearConvert( current_period, claw_period, duaration );
        current_stancePhaseRatio = linear_trans_stancePhaseRatio.linearConvert( current_stancePhaseRatio, 0.8334, duaration );
    ...
    // std::cout<<" current_period :"<< current_period <<std::endl;
    // std::cout<<" current_stancePhaseRatio :"<< current_stancePhaseRatio <<std::endl;
    // std::cout<<" current_bias :"<< current_bias.transpose() <<std::endl;

    if( linear_trans_period.retConvDoneFlag() == false )
    {
        USVLCC_SETZERO = true;
    }
    _ctrlComp->waveGen->GaitChange( current_period, current_stancePhaseRatio, current_bias);
}

void State_Force_Pos_WBC::gaitChange_CPG(void)
{
    if( GAIT_INDEX == 0 )
        _CPG.gaitChange( 0.5, 0.002, Trot_period );
    else if( GAIT_INDEX == 1 )
