...
SixLegDogRobot::SixLegDogRobot(){
    ...
    _Legs[0] = new SixLegDogLeg(0, Vec3( 0.2850, -0.090, 0));//rf  //这里输入的是 pHip2B 20240524
    _Legs[1] = new SixLegDogLeg(1, Vec3( 0.2850,  0.090, 0));//lf
    _Legs[2] = new SixLegDogLeg(2, Vec3(-0.0154, -0.090, 0));//rm
    _Legs[3] = new SixLegDogLeg(3, Vec3(-0.0154,  0.090, 0));//lm
    _Legs[4] = new SixLegDogLeg(4, Vec3(-0.2658, -0.090, 0));//rb
    _Legs[5] = new SixLegDogLeg(5, Vec3(-0.2658,  0.090, 0));//lb

    //lcc 实际上，这个理想位置就是：body系下 x和y值就是(足端+小腿+大腿)平面,即LX和LY+L1。z方向的值还不知道怎么确定

    //hip下的投影
    // _feetPosNormalStand <<  0.2850,  0.2850, -0.0154, -0.0154, -0.2658, -0.2658, 
    //                        -0.16725,  0.16725, -0.16725,  0.16725, -0.16725,  0.16725,
    //                        -0.3180, -0.3180, -0.3180, -0.3180, -0.3180, -0.3180;
    // 中腿在x前后对称
    // _feetPosNormalStand <<  0.2850,  0.2850, 0.0096, 0.0096, -0.2658, -0.2658, 
    //                        -0.16725,  0.16725, -0.16725,  0.16725, -0.16725,  0.16725,
    //                        -0.3180, -0.3180, -0.3180, -0.3180, -0.3180, -0.3180;
    // 中腿在x上调整
    _feetPosNormalStand <<  0.2850,  0.2850, 0.0154, 0.0154, -0.2658, -0.2658, 
    ...
    _robVelLimitYaw << -0.5, 0.5;
        ...
        // _Ib = Vec3(0.0357726630771341, 0.10943725731452, 0.13646430714546).asDiagonal(); //原始的转动惯量
        _Ib = Vec3(1, 1, 1).asDiagonal();//lcc 20240611: 修改后的转的惯量 -> 我发现换上这个以后，往前走也不会沉头了，效果好了很多
}

// ixx="0.0357726630771341"
// ixy="4.9181193647426E-19"
// ixz="-0.00048930434128725"
// iyy="0.10943725731452"
// iyz="2.83926822363752E-18"
// izz="0.13646430714546" />
