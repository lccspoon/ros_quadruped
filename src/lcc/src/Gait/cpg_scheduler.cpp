...
void CPG::gaitChange( double _gait, double _control_cycle, double _T)
{
    gait=_gait;                      
    control_cycle=_control_cycle;
    T=_T;       
}
...
void CPG::cpgNewParaInit(void)
{
    ...
    savedbeta.setZero();
    beta0<< 1,1,1,1,1,1;
    // beta0<< 1.25,1.25,1.25,1.25,1.25,1.25;
    // beta0<< 0.75,0.75,0.75,0.75,0.75,0.75;
...
// Eigen::Matrix<double,2,6> CPG::cpgNewRun(float _gait_bias = gait , float _T = T, float _dt_control_cycle = control_cycle)
Eigen::Matrix<double,2,6> CPG::cpgNewRun()
{
    // gait = _gait_bias;
    // T = _T;
    // control_cycle = _dt_control_cycle;
