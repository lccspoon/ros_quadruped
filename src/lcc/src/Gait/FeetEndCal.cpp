...
Vec3 FeetEndCal::calFootPos(int legID, Vec2 vxyGoalGlobal, float dYawGoal, float phase){
    ...
    //lcc 20240617
    // Vec3 _bodyposition_hip;
    // _bodyposition_hip = _est->getPosition() + _ctrlComp->sixlegdogModel->_feetPosNormalStand.col(legID);
    // _bodyposition_hip =  _robModel->_feetPosNormalStand.col(legID);
    // _bodyposition_hip(2) = 0;
    // _nextStep(0) = _bodyposition_hip(0)*phase + (0)*(1-phase)*_Tswing + _bodyVelGlobal(0)*_Tstance/2 + _kx*(_bodyVelGlobal(0) - vxyGoalGlobal(0));
    // _nextStep(1) = _bodyposition_hip(1)*phase + _bodyVelGlobal(1)*(1-phase)*_Tswing + _bodyVelGlobal(1)*_Tstance/2 + _ky*(_bodyVelGlobal(1) - vxyGoalGlobal(1));
    // _nextStep(2) = 0;
    //origin
    ...
    //lcc 20240617
    // _nextStep(0) += _feetRadius(legID) * cos(0 + _feetInitAngle(legID) + 0);
    // _nextStep(1) += _feetRadius(legID) * sin(0 + _feetInitAngle(legID) + 0);
