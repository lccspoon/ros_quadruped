...
void WaveGenerator::GaitChange(double period, double stancePhaseRatio, Vec6 bias)
{
    _period = period;
...
double WaveGenerator::ret_period(void)
{
    return _period;
}

double WaveGenerator::ret_stancePhaseRatio(void)
{
    return _stRatio;
}

Vec6 WaveGenerator::ret_bias(void)
{
    return _bias;
}
...
void WaveGenerator::calcContactPhase(Vec6 &phaseResult, VecInt6 &contactResult, WaveStatus status, Vec6 cpg_phase, Vec6 cpg_contact)
{
    calcWave(_phase, _contact, status, cpg_phase, cpg_contact);
    ...
    if (status != _statusPast)
    {
        ...
        calcWave(_phasePast, _contactPast, _statusPast, cpg_phase, cpg_contact);
        // two special case
...
void WaveGenerator::calcWave(Vec6 &phase, VecInt6 &contact, WaveStatus status,  Vec6 cpg_phase, Vec6 cpg_contact)
{
    if (status == WaveStatus::WAVE_ALL)
    {
        ...
        for (int i(0); i < 6; ++i)
        {
            ...
            if( GAIT_SCHEDULER_SWITCH_INDEX == 0 )
            {
                ...
                // printf("GAIT_SCHEDUL_DEFAULT_MIT\n");

            }
            else if( GAIT_SCHEDULER_SWITCH_INDEX == 1 )
            {
                _normalT(i) = cpg_phase(i);
                phase(i) = cpg_phase(i);
                if( cpg_contact(i) == 1 )  //stand
                    contact(i) = 1;
                else if( cpg_contact(i) == 0 )  //stand
                    contact(i) = 0;
            }
        }
    }
    ...
    // std::cout<<" _normalT: \n"<< _normalT.transpose() <<std::endl;


    // std::cout<<" contact: \n"<< contact.transpose() <<std::endl;
    // std::cout<<" phase: \n"<< phase.transpose() <<std::endl;
    // std::cout<<" _passT: \n"<< _passT <<std::endl;
    // std::cout<<" _startT: \n"<< _startT <<std::endl;
}
