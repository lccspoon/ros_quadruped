...
#ifndef _MPC_FORCE_POSITION_H_
#define _MPC_FORCE_POSITION_H_
...
class State_MPC_Force_Pos : public FSMState{
public:
    State_MPC_Force_Pos(CtrlComponents *ctrlComp);
    ~State_MPC_Force_Pos();
    void enter();
    void run();
    void exit();
...
private:
    ...
    long long mpc_init_counter;


    // 静态成员函数，作为线程函数  
    static void* threadWrapper(void* arg) {  
        State_MPC_Force_Pos* obj = static_cast<State_MPC_Force_Pos*>(arg);  
        obj->threadFunction();  
        return nullptr;  
    }  
    // 类的成员函数，实际要在线程中运行的代码  
    ...
    //     while (true) {  
    //         // calcGrf();
    //         std::cout << "2Thread is running..." << std::endl;  
